...
import { Activity, Bell, Boxes, ChevronDown, CircleUserRound, LayoutDashboard, Map, MapPinned, Menu, PackageCheck, Recycle, Route as RouteIcon, Settings2, Sparkles, X } from "lucide-react";
...
const navItems = [
  ...
  { href: "/rewards", label: "My rewards", icon: Sparkles },
  { href: "/admin", label: "Admin control", icon: Settings2 },
];
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  const roleLabel = role === "admin" ? "Administrator" : role === "centre" ? "Centre staff" : role === "driver" ? "Driver" : "Citizen";
  const visibleItems = role === "citizen"
    ? navItems.filter((item) => ["/", "/deposits", "/centres", "/hotspots", "/rewards"].includes(item.href))
    : role === "centre"
      ? navItems.filter((item) => ["/", "/deposits", "/centres", "/pickups"].includes(item.href))
      : role === "driver"
        ? navItems.filter((item) => ["/", "/pickups", "/routes"].includes(item.href))
        : navItems.filter((item) => !["/rewards"].includes(item.href));
